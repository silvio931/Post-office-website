...
$veza = new Baza();
$veza->spojiDB();
$upit = "SELECT korisnik_id as id, ime as ime, prezime as prezime FROM korisnik WHERE uloga_id = 2 order by 1";
$rezultat = $veza->selectDB($upit);

$num_rows = mysqli_num_rows($rezultat);
$atributi = array("id", "ime", "prezime");

$xmlDom = new DOMDocument();
header("Content-Type: text/xml");
$xmlDom->appendChild($xmlDom->createElement('moderatori'));
$xmlRootNode = $xmlDom->documentElement;

if ($num_rows === 0) {
    $xmlRowElementNode = $xmlDom->createElement('moderator');
