...
$veza = new Baza();
$veza->spojiDB();
$upit = "SELECT DISTINCT d.ime as ime, COUNT(p.pošiljka_id) as neplacene_posiljke FROM država d left join poštanski_ured pu on d.država_id = pu.država_id left join pošiljka p on pu.poštanski_ured_id = p.završni_ured_id left join račun r on p.račun_id = r.račun_id WHERE p.račun_id IS NOT NULL AND r.datum_plaćanja IS NULL AND r.datum_izdavanja >= '$datumOd' AND r.datum_izdavanja <= '$datumDo' GROUP BY d.ime";
$rezultat = $veza->selectDB($upit);

$num_rows = mysqli_num_rows($rezultat);
$atributi = array("ime", "neplacene_posiljke");
...
if ($num_rows > 0) {
    while ($red = mysqli_fetch_array($rezultat)) {
        $ime = $red['ime'];
        $neplacene_posiljke = $red['neplacene_posiljke'];
